...
def _fast_build_enabled():
    return os.environ.get("AP_FAST_BUILD", "1") != "0"


@contextlib.contextmanager
def _skip_enumeration_redistribute_cost():
    if not _fast_build_enabled():
        yield
        return
...
def _parallel_build_workers():
    raw = os.environ.get("AP_PARALLEL_BUILD")
    if raw is None:
        ...
        try:
            ...
            raise ValueError("AP_PARALLEL_BUILD must be an integer") from exc
        if workers < 1:
            raise ValueError("AP_PARALLEL_BUILD must be >= 1")

    if workers > 1 and torch.cuda.is_initialized():
        logger.info(
            "AP_PARALLEL_BUILD requested %s workers, but CUDA is already "
            "initialized; using serial optimizer build.",
            workers,
        )
        return 1
    return workers
...
class ShardingOptimizer:
    ...
    def _compute_node_edge_costs(self, root_idxs):
        ...
        _FORK_OPT = self
        try:
            workers = _parallel_build_workers()
            if workers <= 1 or len(root_idxs) < 64:
